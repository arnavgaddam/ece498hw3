...
def generate_testbench(problem_id: str) -> str:
    """Generate a simple testbench for the given problem."""
    testbenches = {
        "P3": """`timescale 1ns / 1ps
module testbench;
    reg clk;
    reg rst_n;
    reg start;
    ...
    wire busy;
    wire error;
    i2c_master uut (.clk(clk), .rst_n(rst_n), .start(start), .data_tx(data_tx), .rw(rw), .data_rx(data_rx), .sda(sda), .scl(scl), .busy(busy), .error(error));
    initial begin clk = 0; forever #5 clk = ~clk; end
    initial begin rst_n = 0; start = 0; data_tx = 8'h50; rw = 0; #20 rst_n = 1; #10 start = 1; #10 start = 0; #5000 $finish; end
endmodule""",
    }
    return testbenches.get(problem_id, "")


def solve_with_tools() -> Dict[str, Any]:
    """Solve P3 using the LLM with tool-augmented verification.
    ...
    initial_response = instructor_client.chat.completions.create(
        ...
        response_model=P3Solution,
        temperature=DEFAULT_TEMPERATURE,
    )

    initial_solution = initial_response.model_dump()
    initial_code = initial_solution.get("i2c_master_module", "")
    ...
    for turn in range(MAX_TOOL_TURNS):
        print(f"[TOOL LOOP {turn + 1}] Sending request to LLM...")
        ...
        if message.tool_calls:
            ...
            print(f"[TOOL LOOP {turn + 1}] LLM called tool: {func.name}")
            ...
            if func.name == "run_verilog_simulation":
                ...
                if sim_result["passed"]:
                    print(f"[TOOL LOOP {turn + 1}] Simulation passed, using this code")
                    final_content = module_code
                    break
                else:
                    print(f"[TOOL LOOP {turn + 1}] Simulation failed, continuing loop")
        else:
            content = message.content or ""
            print(
                f"[TOOL LOOP {turn + 1}] No tool calls, got text response ({len(content)} chars)"
            )
            final_content = content

    if final_content is None:
        final_content = initial_code
        print("[TOOL LOOP] No final content, using initial code")
...
Output JSON with field 'i2c_master_module' containing the Verilog code. Module MUST be named 'i2c_master'. Use Verilog-2005 only - no SystemVerilog types or unsized literals."""
    ...
    print(
        f"[FINAL] Code preview:\n{final_solution.get('i2c_master_module', '')[:200]}..."
    )
    print()
...
def main():
    print("=" * 60)
    print("Tool-Augmented Pipeline for P3 (I2C Master Controller)")
    print("=" * 60)

    solution = solve_with_tools()

    print("=" * 60)
    print("[VERIFY] Running verifier...")
    print("=" * 60)
